...
if __name__ == "__main__":
    ...
    # dataloader
    src_dataloader = torch.utils.data.DataLoader(src_dataset, batch_size=args.batch_size, shuffle=False, sampler=None, batch_sampler=None, num_workers=args.num_workers, pin_memory=True, drop_last=True, timeout=0, worker_init_fn=None)

    src_dataloader_adv = torch.utils.data.DataLoader(src_dataset_adv, batch_size=args.batch_size, shuffle=False, sampler=None, batch_sampler=None, num_workers=args.num_workers, pin_memory=True, drop_last=True, timeout=0, worker_init_fn=None)
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch_idx, (batch_src, batch_src_adv) in enumerate(zip(src_dataloader, src_dataloader_adv)):
            ...
            if type(imgs_src) is list and len(imgs_src) > 0:
                imgs_src = torch.cat(imgs_src, dim=0)
            
            if type(lbls_src) is list and len(lbls_src) > 0:
                lbls_src = torch.cat(lbls_src, dim=0)

            if type(imgs_src_adv) is list and len(imgs_src_adv) > 0:
                imgs_src_adv = torch.cat(imgs_src_adv, dim=0)
            
            if type(lbls_src_adv) is list and len(lbls_src_adv) > 0:
                lbls_src_adv = torch.cat(lbls_src_adv, dim=0)
